...
const ExploreInstrumentDetail = ({ hit }) => {
  ...
  return (
    <div className="flex flex-col w-full h-full justify-between">
        ...
        <p className="font-extrabold text-lg text-explore-blue">{hit.name.length > 25 ? `${hit.name.slice(0,25)}...` : hit.name}</p>
        <button className="bg-explore-blue text-white font-bold self-center w-10/12 p-2 rounded-lg translate-y-3">Follow</button>
    </div>
  )
}
