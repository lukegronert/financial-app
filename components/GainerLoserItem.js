      ...
      {status === "gainer" ? (
        <>
          ...
          <div className="flex flex-row gap-2 items-center">
            <span className="text-green-400 bg-green-100 font-bold text-sm px-1 py-0.5">{`+ ${changePercentage}`}</span>
        ...
        <>
          ...
          <span className="text-explore-blue font-bold">{`$${data.price}`}</span>
          <div className="flex flex-row gap-2 items-center">
            <span className="text-red-400 bg-red-100 font-bold text-sm px-1 py-0.5">{`- ${changePercentage}`}</span>
            <span className="text-gray-400 font-bold text-sm">{`- $${change}`}</span>
          </div>
        </>
      )}
