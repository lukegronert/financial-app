...
const GainerLoserItem = ({ data, status }) => {
  ...
  return (
    <div className="flex flex-col flex-1 justify-evenly bg-white rounded-xl p-2 gap-1 cursor-pointer" onClick={() => router.push(`/instruments/${data.name}/${data.symbol}`)}>
          <h2 className="flex-1 text-explore-blue font-extrabold w-full h-full">{data.name}</h2>
          <span className="text-explore-blue font-bold">{`$${data.price.toFixed(2)}`}</span>
